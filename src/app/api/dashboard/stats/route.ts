...
export async function GET() {
  try {
    ...
    const [
      ...
      prisma.plant.count({ where: { userId: user.id, isArchived: { not: true } } }),
      prisma.plant.count({ where: { userId: user.id, healthStatus: 'healthy', isArchived: { not: true } } }),
      prisma.breedingRecord.count({ where: { userId: user.id } }),
      prisma.cloneBatch.count({ where: { userId: user.id } }),
      prisma.seedBatch.count({ where: { harvest: { breedingRecord: { userId: user.id } } } }),
      prisma.seedling.count({ where: { seedBatch: { harvest: { breedingRecord: { userId: user.id } } } } })
    ])

    // === BREEDING PIPELINE STATS (filtered by user) ===
    // Seed batches by status
    const seedBatchesByStatus = await prisma.seedBatch.groupBy({
      by: ['status'],
      where: { harvest: { breedingRecord: { userId: user.id } } },
      _count: true
    })
    ...
    const seedlingsByStatus = await prisma.seedling.groupBy({
      by: ['selectionStatus'],
      where: { seedBatch: { harvest: { breedingRecord: { userId: user.id } } } },
      _count: true
    })
    ...
    const recentCrosses = await prisma.breedingRecord.count({
      where: { userId: user.id, crossDate: { gte: thirtyDaysAgo } }
    })

    // Crosses with active germination
    const activeSeedBatches = await prisma.seedBatch.count({
      where: { harvest: { breedingRecord: { userId: user.id } }, status: { in: ['SOWN', 'GERMINATING'] } }
    })
    ...
    const graduatedSeedlings = await prisma.seedling.count({
      where: { seedBatch: { harvest: { breedingRecord: { userId: user.id } } }, selectionStatus: 'GRADUATED' }
    })

    // === CLONE BATCH STATS (filtered by user) ===
    ...
    const totalClonesInProgress = await prisma.cloneBatch.aggregate({
      ...
      where: { userId: user.id, status: { notIn: ['COMPLETE', 'FAILED'] } }
    })
    ...
    // === PLANT HEALTH DISTRIBUTION (filtered by user) ===
    const healthDistribution = await prisma.plant.groupBy({
      by: ['healthStatus'],
      _count: true,
      where: { userId: user.id, isArchived: { not: true } }
    })

    // === SPECIES/SECTION DISTRIBUTION (filtered by user) ===
    ...
    // Recent crosses (filtered by user)
    ...
    // Recent seed batches (filtered by user)
    ...
    // Recent plants added (filtered by user)
    ...
    // === KEEPERS/HOLDBACKS COUNT (filtered by user) ===
    const keeperCount = await prisma.seedling.count({
      where: { seedBatch: { harvest: { breedingRecord: { userId: user.id } } }, selectionStatus: 'KEEPER' }
    })
    const holdbackCount = await prisma.seedling.count({
      where: { seedBatch: { harvest: { breedingRecord: { userId: user.id } } }, selectionStatus: 'HOLDBACK' }
    })

    // === SUBSTRATE HEALTH ANALYSIS (filtered by user) ===
