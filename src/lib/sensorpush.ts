...
async function fetchWithTimeout(
  ...
  try {
    ...
    if (error instanceof Error && error.name === 'AbortError') {
      throw new Error(`SensorPush API request timed out after ${timeoutMs}ms`);
    }
    throw error;
  } finally {
...
async function getAuthorizationCode(email: string, password: string): Promise<string> {
  const response = await fetchWithTimeout(`${SENSORPUSH_API_BASE}/oauth/authorize`, {
    method: 'POST',
...
async function getAccessToken(authorizationCode: string): Promise<string> {
  const response = await fetchWithTimeout(`${SENSORPUSH_API_BASE}/oauth/accesstoken`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function getSensors(): Promise<Record<string, SensorPushSensor>> {
  ...
  const response = await fetchWithTimeout(`${SENSORPUSH_API_BASE}/devices/sensors`, {
    method: 'POST',
    headers: {
...
export async function getSamples(
  ...
  const response = await fetchWithTimeout(`${SENSORPUSH_API_BASE}/samples`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'Authorization': token,
